...
import {
  ...
  ArrowRight, ShieldAlert, Check, Calendar, FileText, Sparkles, Loader2, Star } from
'lucide-react';
...
const LocationMarker = ({ position, setPosition }) => {
  ...
  // Update map center when position changes externally (e.g. geolocation)
  React.useEffect(() => {
    if (position) {
...
const CarritoPage = () => {
  ...
  const [direccion, setDireccion] = useState({ lat: -17.7833, lng: -63.1821 }); // Santa Cruz default
  ...
  const totalPagarAhora = subtotal;
  ...
  const handleConfirmarPedido = async () => {
    if (modalidadEntrega === 'envio' && !direccion) {
      toast.error('Por favor, seleccioná una ubicación de entrega en el mapa.');
      return;
    }
    if (!aceptaTerminos) {
    ...
    try {
      ...
      const payloadDireccion = modalidadEntrega === 'retiro' ? 'Retiro en finca' : JSON.stringify(direccion);
  ...
  const handleSimularPagoExitoso = () => {
    ...
    setTimeout(() => {
      ...
      toast.success("\xA1Pago registrado! Tu pedido est\xE1 confirmado");
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              return (
                <div key={itemId} className="bg-white rounded-2xl border border-blue-100 shadow-[0_1px_3px_rgba(59,130,246,0.08)] p-5 flex gap-4 hover:shadow-md transition-all animate-slide-up">
                    {/* Mini photo */}
                    <div className="w-20 h-20 bg-slate-50 border border-slate-100 rounded-xl overflow-hidden shrink-0 flex items-center justify-center relative">
                      {console.log("Cargando imagen en carrito:", item.foto_url)}
                      {item.foto_url ?
                        <>
                          <img
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="mt-2 space-y-1">
                              ...
                              <p className="text-[10px] text-amber-600 font-semibold leading-tight">
                                * Este producto se entregará en la fecha de disponibilidad.
                              </p>
                            </div>
                        }
                        </div>
                ...
                <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl p-3 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-600">
                      Productor verificado <Check size={16} className="inline-block mr-1" />
                    </p>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-100 p-6 space-y-5">
                ...
                modalidadEntrega === 'retiro' ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-white hover:border-emerald-300'}`
                    ...
                    <div className="text-lg mb-1"><Star size={16} className="inline-block mr-1" /></div>
                ...
                modalidadEntrega === 'envio' ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white hover:border-blue-300'}`
                    ...
                    <div className="text-xs space-y-1.5 font-semibold text-slate-700">
                      <p className="flex items-center gap-1">
                        <span className="text-lg"><Star size={16} className="inline-block mr-1" /></span> Costo coordinado con transportista
                      </p>
                      <p className="flex items-center gap-1">
                        <span className="text-lg">⏱<Star size={16} className="inline-block mr-1" /></span> Tiempo estimado: 1-3 días
                      </p>
                    </div>
                  </label>
                </div>
                ...
                {modalidadEntrega === 'envio' && (
                  <div className="space-y-4 mt-6 animate-slide-up bg-slate-50 p-5 rounded-2xl border border-slate-100">
                    <div className="flex items-center justify-between">
                      <label className="block text-xs font-black text-slate-800 uppercase tracking-wider">Ubicación de entrega *</label>
                      <button 
                        onClick={() => {
                          if (navigator.geolocation) {
                            navigator.geolocation.getCurrentPosition(
                              (pos) => setDireccion({ lat: pos.coords.latitude, lng: pos.coords.longitude }),
                              () => toast.error('No se pudo obtener la ubicación. Revisá los permisos de tu navegador.')
                            );
                          } else {
                            toast.error('Tu navegador no soporta geolocalización.');
                          }
                        }}
                        className="flex items-center gap-1.5 text-xs font-bold text-blue-600 hover:text-blue-700 bg-blue-100 hover:bg-blue-200 px-3 py-1.5 rounded-lg transition-colors"
                      >
                        <Navigation className="w-4 h-4" /> Usar mi ubicación
                      </button>
                    </div>
                    
                    <div className="relative h-64 w-full rounded-xl overflow-hidden border-2 border-slate-200 shadow-inner z-10">
                      <MapContainer 
                      ...
                      >
                        ...
                        <LocationMarker position={direccion} setPosition={setDireccion} />
                      </MapContainer>
                      <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-slate-900/80 backdrop-blur-sm text-white text-[10px] font-bold px-3 py-1.5 rounded-full z-[400] shadow-lg pointer-events-none">
                        Mové el mapa o hacé clic para ubicar el pin
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="md:col-span-1">
              <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-md space-y-6 sticky top-24">
                ...
                disabled={procesando || !aceptaTerminos || (modalidadEntrega === 'envio' && !direccion)}
